...
def get_next_node_up_based_on_cond(start_node, get_node_by_output_name, *, node_found_cond, node_end_search_cond=None):
    """
...
def get_slice_info(slice_node, get_node_by_output_name):
...
def get_node_input_constant_op_value(node, get_node_by_output_name):
    """
    ...
    if node.input[0] in get_node_by_output_name.keys() and get_node_by_output_name[node.input[0]].op_type == "Constant":
        shape_const_node = get_node_by_output_name[node.input[0]]
    elif get_node_by_output_name[node.input[1]].op_type == "Constant":
        shape_const_node = get_node_by_output_name[node.input[1]]
    else:
        raise ValueError(f"Expecting one of the reshape's input type to be Constant, \
                         but got {get_node_by_output_name[node.input[0]].op_type} and \
                            {get_node_by_output_name[node.input[1]].op_type}")

    tensor = shape_const_node.attribute[0].t
    np_dtype = helper.tensor_dtype_to_np_dtype(tensor.data_type)
    const_tensor_value = np.frombuffer(tensor.raw_data, dtype=np_dtype).reshape(tensor.dims)
    return const_tensor_value
