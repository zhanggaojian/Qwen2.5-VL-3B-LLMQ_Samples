...
from mha2sha.utils.logger import log_debug, log_warning
...
class RopeExtension:
    ...
    def create_llama_rope_node(self,
        ...
        if self.mha2sha_optim.nchw_aligned:
            ...
            _1st_half_slice, _1st_slice_init_list = self.mha2sha_optim._op_factory.get_slice_op(inp, start=0, end=head_dim//2, axis=1)
            _2nd_half_slice, _2nd_slice_init_list = self.mha2sha_optim._op_factory.get_slice_op(inp, start=head_dim//2, end=head_dim, axis=1)
    ...
    def get_mha_rope_nodes(self, linear_node, qk_matmul_node) -> Optional[RopeNode]:
        ...
        try:
            ...
            start, end, axes, step = get_slice_info(temp_slice_1, self.mha2sha_optim.get_node_by_output_name)
            if start == 0:
