...
def generate_target_artifacts(text_encoder, unet, controlnet, tokenizer, scheduler, config, diffusion_steps=[20, 50], guidance_scales=[5.0, 7.5, 10.0], seed_list=[10,50,100], min_seed=0, max_seed=2229135949491605, generator_seed=0):
    ...
    if isinstance(scheduler, DPMSolverMultistepScheduler):
        ...
        for idx in range(diffusion_steps[0]):
            noise_path = os.path.join(save_path, f"random_noise_{idx}.bin")
            generator = torch.Generator('cpu').manual_seed(generator_seed)
            random_noise = randn_tensor(unet_out_shape, generator=generator).detach().cpu().numpy()
    ...
    for n_seeds in seed_list:
        ...
        for current_seed in seeds:
            ...
            latents = torch.randn((batch_size, unet.config.in_channels, 64, 64), generator=generator)
    ...
    scheduler_time_steps_root = os.path.join(save_path, 'scheduler_time_steps')
    os.makedirs(scheduler_time_steps_root, exist_ok=True)
    ...
    for num_steps in diffusion_steps:
        ...
        scheduler_time_steps_path = os.path.join(scheduler_time_steps_root, f'scheduler_time_steps_{num_steps}.pkl')
