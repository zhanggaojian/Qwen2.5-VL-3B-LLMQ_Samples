...
## 四·补 · KV Cache 与 attention mask 如何配合（常见面试点）

**一句话**：KV Cache 和 attention mask 是**分工不同、必须一起用**的两个机制——**KV Cache 决定"有哪些东西可以被看"（提供 K/V 候选），mask 决定"这些东西里哪些真的允许看"（可见性）**。不是谁包含谁。

### 1. 各管一摊

| | KV Cache | attention mask |
|---|----------|----------------|
| 管什么 | 提供**被看的内容**：历史 + 当前所有 token 的 K/V | 决定当前 query **能不能看**某个位置（0=可见，`mask_neg`=屏蔽）|
| 类比 | 图书馆里摆着的所有书 | 借阅权限表：哪些书这个人能翻 |

KV Cache 只负责把历史 K/V "端上桌"，"哪些历史该看、哪些是凑数的" 它不管——那是 mask 的活。

### 2. 维度上就绑定了：mask 的列数 = KV Cache 的"历史+当前"总长

注意力打分是 `Q(当前) × K(历史+当前)`，结果形状 `[tgt, src]`；mask 要逐元素加上去，**形状也必须是 `[tgt, src]`**，即 **mask 的 `src` 维必须正好覆盖 KV Cache 里"历史 + 当前"的总长度**。所以 KV Cache 变长，mask 的 `src` 维就得跟着一样长——这是两者"绑一起"的根本原因。

### 3. 定长场景下 mask 要挡"两类东西"（最能说明缺一不可）

本项目 combined mask 形状 `[B,1,1073,2048]`，其中 `2048 = 历史975 + 当前1073`。它屏蔽两类**完全不同**的位置：

```
mask 一行 = [ dummy历史(如675)→屏蔽 | 真实历史(如300)→放行0 | 当前段→下三角 ]
              ↑① 挡 KV Cache 的填充槽        ↑ 历史全放行        ↑② 挡未来
```

1. **KV Cache 里的 dummy 历史**：端侧把历史槽**定长预分配成 975**，真实历史可能只有 300，其余 675 是**填充的 0 向量**（见第四节 dummy KV、[05 笔记](./05-通用前向处理流程.md) 4.5）。它们不是真实历史，当前 token 绝不能 attend → **靠 mask 把这些列设成 `mask_neg` 挡掉**。
2. **当前段内的未来 token**：当前 1073 个 token 之间仍是因果的 → **下三角**（`_make_causal_mask` 那段，[附录E](./02-附录E-端侧定长与计算图导出.md)）。

> **看清关系了**：没有 KV Cache 时，mask 只需管当前段的因果；**一旦用了 KV Cache（尤其定长预分配），mask 就多了一个职责——把 KV Cache 里"凑数的 dummy 历史"标记为不可见**。这就是两者配合最直接的体现。历史里真实的部分 mask 一律放行(0)，因为历史都是"过去"。

### 4. 记这句

**KV Cache 给"内容"、mask 给"可见性"，两者在 `[tgt, src]` 形状上对齐、缺一不可**：mask 在这些候选上盖章——**真实历史放行、dummy 历史屏蔽、当前未来屏蔽**。
...
- **与 attention mask 配合**（见 4·补）：KV Cache 给"内容"、mask 给"可见性"，两者在 `[tgt, src]` 形状对齐、缺一不可；用了 KV Cache 后 mask 多一个职责——除了挡"未来"，还要挡 KV Cache 里"凑数的 dummy 历史"（真实历史放行 0）。
